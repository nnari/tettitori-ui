...
import { Card, Segment, Header, Button, TextArea, Form, Container } from 'semantic-ui-react';
...
export const JobEditView = ({ jobs }) => {
  ...
  return (
    <Container style={{ marginTop: '6em' }}>
      ...
      <Button color='green' onClick={() => dispatch({type: 'add'})}>Lisää uusi tettipaikka</Button>
      {
        state.map((job, idx) => (
          <Segment key={idx}>
            <Form>
              {job.title}
              <Button color='red' floated='right' onClick={() => dispatch({type: 'remove', payload: job})}>Poista tämä tettipaikka</Button>
            </Form>
